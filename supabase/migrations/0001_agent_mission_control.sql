...
create table if not exists copilots (
  ...
  slug text not null unique,
  description text not null default '',
  runtime text not null check (runtime in ('langgraph','openai-assistants','anthropic-sdk','gemini','custom')),
  status text not null check (status in ('active','paused','draft','degraded','archived')),
  production_version_id text,
  latest_version_id text not null,
  model text not null,
  model_provider text not null check (model_provider in ('openai','anthropic','google','mistral','local')),
  owner text not null,
