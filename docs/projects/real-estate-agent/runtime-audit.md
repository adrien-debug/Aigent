...
| `create-agent-form.tsx` (UI de création) | `src/components/agent-ops/create-agent-form.tsx:17,32` | `RUNTIME_OPTIONS` propose les 4 valeurs dans un `<select>` ; `DEFAULT_RUNTIME = 'langgraph'` est le défaut du formulaire (commentaire ligne 30-31 : *"langgraph is the only runtime with a real execution engine"*), mais l'utilisateur peut changer la sélection librement. | UI = défaut sain, mais pas d'imposition. |
