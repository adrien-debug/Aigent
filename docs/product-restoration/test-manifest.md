...
| `check:dev-port` | 3987 épinglé dans **4 resolvers** ; aucun des trois ports bannis (liste dans `AGENTS.md` § « Port de dev ») en code vif ou doctrine ; échoue si un resolver a disparu. La gate n'exempte pas la prose : citer un numéro banni dans un `.md` la fait rougir — d'où le renvoi plutôt que la liste | qu'un serveur tourne réellement sur 3987 — gate **statique** |
