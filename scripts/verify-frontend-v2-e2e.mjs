#!/usr/bin/env node
/**
 * Frontend V2 runtime E2E — drives a real browser against /admin-v2/runs and
 ...
 *
 * Run: npm run verify:frontend-v2-e2e
 */
import { chromium } from 'playwright'

const BASE = process.env.V2_TEST_URL ?? 'http://localhost:3210'
const RUNS_URL = `${BASE}/admin-v2/runs`
...
async function main() {
  ...
  try {
    ...
    check(response?.status() === 200, `GET /admin-v2/runs answered ${response?.status()}, expected 200`)
    ...
    // --- 2. /admin-v2 redirects to the cockpit -----------------------------
    await page.goto(`${BASE}/admin-v2`, { waitUntil: 'networkidle' })
    check(
      new URL(page.url()).pathname === '/admin-v2/runs',
      `/admin-v2 should redirect to /admin-v2/runs, landed on ${page.url()}`
    )

    // --- 3. No legacy shell leaked into the V2 slice -----------------------
    const legacyNav = await page.locator('nav[aria-label="Main"]').count()
    check(legacyNav === 0, 'legacy dashboard sidebar (nav[aria-label="Main"]) is present in V2')
  ...
  console.log('✓ frontend V2 E2E passed — /admin-v2/runs renders, filters narrow + persist in the URL, every link resolves, shell holds at 1440/1280/1024/375, zero console errors.')
}
