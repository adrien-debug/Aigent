...
| --- | --- | --- |
| `check:legacy-design-doctrine` | doc `cockpit-catalyst-migration.md` marquée abrogée ; `scripts.check` sans gates layout supprimées ; pas de réinjection DS Guardian / `check:ds` / `check:catalyst` dans les modules d'injection ; règle explicite dans `AGENTS.md` ; scan Factory des surfaces reconstruites contre les couleurs structurelles interdites (`zinc-*`, `gray-*`, `slate-*`, `dark:*`, `bg/text white/black`, `border/fill/stroke white/black`, `hex`, `rgb/rgba/hsl`) avec échec si 0 cible scannée | la qualité des pixels, la légitimité des surfaces, la hiérarchie visuelle, le responsive, l'accessibilité complète ; la justesse d'une exception média non marquée (`DS_FACTORY_MEDIA_EXCEPTION`) |
