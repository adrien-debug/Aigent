...
| --- | --- | --- |
| `npm run check` | typecheck · lint:fast · lint · no-legacy-front · **no-legacy-design-governance** · **production-visual-authority** · **ui-kit-integrity** · agent-truth · lifecycle-truth · registry-parity · registry-integrity · dev-port · render-truth · rsc-boundary · **schema-rebuildable** · secrets · audit:dead | CI (`.github/workflows/ci.yml`) + pré-livraison |
...
| --- | --- | --- |
| `check:no-legacy-design-governance` | anti-retour doctrine : doc historique marquée abrogée, `scripts.check` sans `check:ds` / `check:catalyst`, pas de réinjection « Design System Guardian », zéro-scroll obligatoire, viewport lock, sync externe de gouvernance ou quotas UI arbitraires | la cohérence visuelle des surfaces produit ; le rendu pixel-perfect ; l'accessibilité à l'exécution |
| `check:production-visual-authority` | scan des seules surfaces de production ; refuse accents Tailwind bruts en contexte statut métier, couleurs littérales hors autorité thème, redéfinition locale de `RUN_STATUS_COLOR`; échoue si 0 cible ; affiche fichiers scannés, exclus et raisons | la qualité visuelle globale, le layout, les zones Lab/Composer/prototypes, les médias/visualisations externes explicitement exclus |
...
| `check:ui-kit-integrity` | la **substance** de `src/components/ui/` : les 14 primitives existent et exportent les 43 symboles consommés (liste dérivée des imports réels du produit, donc elle ne peut plus prendre de retard) ; `TouchTarget` garde sa cible tactile de 44 px en contexte tactile (`max(100%,2.75rem)` + `pointer-fine:hidden`) ; les marqueurs d'accessibilité sont présents (`forced-colors`, `data-disabled`, anneau de focus, nom accessible du lien de ligne) ; **aucune couleur Tailwind brute** dans le kit | **que le rendu soit bon**. Elle lit du texte, pas des pixels : un anneau de focus présent mais invisible, un contraste insuffisant ou une primitive cassée passent. Elle ne dit pas non plus que les écrans utilisent le kit. Ce n'est plus une empreinte SHA-256 : le gel interdisait l'évolution légitime (dont la migration vers `--aig-*`) et se contournait par un `--update` réflexe — voir `src/components/ui/README.md` |
