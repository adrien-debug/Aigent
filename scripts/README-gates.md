...
| --- | --- | --- |
| `check:legacy-design-doctrine` | doc `cockpit-catalyst-migration.md` marquée abrogée ; `scripts.check` sans gates layout supprimées ; pas de réinjection DS Guardian / `check:ds` / `check:catalyst` dans les modules d'injection ; règle explicite dans `AGENTS.md` ; scan Factory de 90 fichiers — surfaces reconstruites **et kit `src/components/ui/**`** contre les couleurs structurelles interdites (`zinc-*`, `gray-*`, `slate-*`, `dark:*`, `bg/text white/black`, `border/fill/stroke white/black`, `hex`, `rgb/rgba/hsl`) **et les accents Tailwind bruts** (`sky-*`, `amber-*`, `red-*`, `emerald-*`… — un état se dit avec `--aig-severity-*`), avec échec si 0 cible scannée | la qualité des pixels, la légitimité des surfaces, la hiérarchie visuelle, le responsive, l'accessibilité complète ; la justesse d'une exception média non marquée (`DS_FACTORY_MEDIA_EXCEPTION`) |
...
| `check:ui-kit-integrity` | la **substance** de `src/components/ui/` : les 14 primitives existent et exportent les 43 symboles consommés (liste dérivée des imports réels du produit, donc elle ne peut plus prendre de retard) ; `TouchTarget` garde sa cible tactile de 44 px (`max(100%,2.75rem)`) ; les marqueurs d'accessibilité sont présents (`forced-colors`, `data-disabled`, anneau de focus, nom accessible du lien de ligne) ; **aucune couleur Tailwind brute** dans le kit (autorité visuelle unique) | **que le rendu soit bon**. Elle lit du texte, pas des pixels : un anneau de focus présent mais invisible, un contraste insuffisant ou une primitive cassée passent. Elle ne dit pas non plus que les écrans utilisent le kit. Ce n'est plus une empreinte SHA-256 : le gel interdisait l'évolution légitime (dont la migration vers `--aig-*`) et se contournait par un `--update` réflexe — voir `src/components/ui/README.md` |
