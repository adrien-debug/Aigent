#!/usr/bin/env node
/**
 * Rebuilt-surface boundary guard — keeps the rebuilt admin screens INSIDE the kit.
 *
...
const V2_DIRS = [
  join(SRC, 'components', 'admin-dashboard'),
  join(SRC, 'components', 'admin-shell'),
  join(SRC, 'components', 'runs-console'),
  join(SRC, 'lib', 'runs-console'),
]
  ...
  if (violations.length > 0) {
    ...
    console.error(
      '\nFrontend V2 boundary guard FAILED.\n' +
        'The rebuilt admin surfaces (components/admin-dashboard, admin-shell, runs-console and ' +
