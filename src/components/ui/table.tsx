      ...
      <div className="flow-root">
        <div {...props} className={clsx(className, '-mx-(--gutter) overflow-x-auto whitespace-nowrap')}>
          <div className={clsx('inline-block min-w-full align-middle', !bleed && 'sm:px-(--gutter)')}>
            <table className="min-w-full text-left text-sm/6 text-(--aig-text)">{children}</table>
          </div>
        </div>
      </div>
...
export function TableHead({ className, ...props }: Readonly<React.ComponentPropsWithoutRef<'thead'>>) {
  return <thead {...props} className={clsx(className, 'text-(--aig-text-muted)')} />
}
    ...
    <TableRowContext.Provider value={{ href, target, title } as React.ContextType<typeof TableRowContext>}>
      <tr
        ...
        className={clsx(
          className,
          href &&
            'has-[[data-row-link][data-focus]]:outline-2 has-[[data-row-link][data-focus]]:-outline-offset-2 has-[[data-row-link][data-focus]]:outline-(--aig-accent) focus-within:bg-(--aig-line-soft)',
          striped && 'even:bg-(--aig-line-soft)',
          href && striped && 'hover:bg-(--aig-line-soft)',
          href && !striped && 'hover:bg-(--aig-line-soft)'
        )}
      />
...
export function TableHeader({ className, ...props }: Readonly<React.ComponentPropsWithoutRef<'th'>>) {
  ...
  return (
    <th
      ...
      className={clsx(
        className,
        'border-b border-b-(--aig-line) px-4 py-2 font-medium first:pl-(--gutter,--spacing(2)) last:pr-(--gutter,--spacing(2))',
        grid && 'border-l border-l-(--aig-line) first:border-l-0',
  ...
  return (
    <td
      ...
      className={clsx(
        ...
        !striped && 'border-b border-(--aig-line)',
        grid && 'border-l border-l-(--aig-line) first:border-l-0',
        dense ? 'py-2.5' : 'py-4',
        !bleed && 'sm:first:pl-1 sm:last:pr-1'
      )}
