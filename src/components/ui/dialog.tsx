  ...
  return (
    <Headless.Dialog {...props}>
      <Headless.DialogBackdrop
        transition
        className="fixed inset-0 flex w-screen justify-center overflow-y-auto bg-(--aig-scrim) px-2 py-2 transition duration-100 focus:outline-0 data-closed:opacity-0 data-enter:ease-out data-leave:ease-in sm:px-6 sm:py-8 lg:px-8 lg:py-16"
      />
        ...
        <div className="grid min-h-full grid-rows-[1fr_auto] justify-items-center sm:grid-rows-[1fr_auto_3fr] sm:p-4">
          <Headless.DialogPanel
            ...
            className={clsx(
              className,
              sizes[size],
              'row-start-2 w-full min-w-0 rounded-t-lg bg-(--aig-raised) p-(--gutter) shadow-(--shadow-lg) ring ring-(--border-default) [--gutter:--spacing(8)] sm:mb-auto sm:rounded-lg forced-colors:outline',
...
export function DialogTitle({
  ...
  return (
    <Headless.DialogTitle
      {...props}
      className={clsx(className, 'text-lg/6 font-semibold text-balance text-(--aig-text) sm:text-base/6')}
    />
  )
}
