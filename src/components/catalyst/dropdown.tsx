...
export function DropdownItem({
  ...
  let classes = clsx(
    ...
    // Focus
    // Single-accent design system: the focused item takes the brand green with
    // dark text (white on accent-500 is illegible — see button.tsx accent).
    'data-focus:bg-accent-500 data-focus:text-zinc-950',
    // Disabled state
    ...
    '*:data-[slot=icon]:text-zinc-500 data-focus:*:data-[slot=icon]:text-zinc-950 dark:*:data-[slot=icon]:text-zinc-400 dark:data-focus:*:data-[slot=icon]:text-zinc-950',
    // Avatar
...
export function DropdownDescription({
  ...
  return (
    <Headless.Description
      ...
      className={clsx(
        className,
        'col-span-2 col-start-2 row-start-2 text-sm/5 text-zinc-500 group-data-focus:text-zinc-900 sm:text-xs/5 dark:text-zinc-400 forced-colors:group-data-focus:text-[HighlightText]'
      )}
    />
  )
}
    ...
    >
      {(Array.isArray(keys) ? keys : keys.split('')).map((char, index) => (
        <kbd
          key={index}
          className={clsx([
            'min-w-[2ch] text-center font-sans text-zinc-400 capitalize group-data-focus:text-zinc-900 forced-colors:group-data-focus:text-[HighlightText]',
