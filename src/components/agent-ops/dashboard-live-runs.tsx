      ...
      {runs.length > 0 ? (
        <ul className="divide-y divide-zinc-950/5 border-t border-zinc-950/5">
          {runs.map((run) => (
            <li
              key={run.id}
              className="grid min-h-14 grid-cols-[minmax(0,1fr)_3.5rem_3.5rem_4.5rem] items-center gap-2 px-4 py-2.5 hover:bg-(--color-surface-focus) sm:grid-cols-[minmax(0,1fr)_4.5rem_4.5rem_7rem]"
            >
              ...
              <span className="truncate text-right font-mono text-xs tabular-nums text-zinc-500" title="Duration">
                {formatDurationMs(run.latencyMs)}
              </span>
              <span className="truncate text-right font-mono text-xs tabular-nums text-zinc-500" title="Cost">
                {run.costUsd === null ? '—' : formatUsd(run.costUsd)}
              </span>
              <span
                className="truncate text-right font-mono text-xs tabular-nums text-zinc-400"
