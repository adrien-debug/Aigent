...
export function telemetryWrapperSource(manifest: AgentManifest, copilot?: Copilot): string {
  ...
  const quotedDefault = (value: string): string => (value ? '`' + value + '`' : 'undefined')
  const endpointLiteral = quotedDefault(endpointDefault)
  const projectIdLiteral = quotedDefault(projectIdDefault)
  const agentIdLiteral = quotedDefault(agentIdDefault)
  const agentVersionLiteral = quotedDefault(agentVersionDefault)
  const modelLiteral = quotedDefault(modelDefault)
...
const DEFAULT_CONFIG: TelemetryConfig = {
  ...
  agentVersion: ${agentVersionLiteral},
  model: ${modelLiteral},
...
function readme(copilot: Copilot, manifest: AgentManifest): string {
  const allowedRoutes = manifest.allowedRoutes.length
    ? manifest.allowedRoutes.map((route) => '`' + route + '`').join(', ')
    : '_none declared_'
  const forbiddenActions = manifest.forbiddenActions.length
    ? manifest.forbiddenActions.map((action) => '`' + action + '`').join(', ')
    : '_none declared_'

  const telemetryFileLine = manifest.telemetry
...
- \`manifest.json\` — the serialized agent manifest (prompt, guardrails, contract).${telemetryFileLine}
