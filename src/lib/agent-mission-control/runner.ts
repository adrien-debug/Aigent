...
/**
 * Thrown when a caller re-uses a `clientRunId` that already exists for this
 * copilot. The run is NOT executed; the caller can retry with a fresh idempotency
 * key or wait for the existing run to complete.
 */
export class DuplicateClientRunIdError extends Error {
  readonly name = 'DuplicateClientRunIdError'
  readonly runId: string
  readonly clientRunId: string

  constructor(clientRunId: string, runId: string) {
    super(`clientRunId ${clientRunId} is already in use by run ${runId}`)
    this.runId = runId
    this.clientRunId = clientRunId
  }
}

interface ReserveAgentRunArgs {
  runId: string
  copilotId: string
  clientRunId: string
  versionId: string
...
/**
 * Reserve a running `agent_runs` row before any model work. When `clientRunId`
 * is already in use, the unique index rejects the insert and we return the
 * existing run id instead of executing. This shrinks the race window from
 * "execute then insert" to "insert then execute".
 */
async function reserveAgentRun(args: ReserveAgentRunArgs): Promise<ReserveAgentRunResult> {
  const { runId, copilotId, clientRunId, versionId, projectId, userLabel, startedAt, inputSummary, modelProvider } = args
  ...
  try {
    ...
    if (err instanceof PgrestError && err.status === 409) {
      const rows = await pgrest<Record<string, unknown>[]>(
        'GET',
        `agent_runs?copilot_id=eq.${encodeURIComponent(copilotId)}&client_run_id=eq.${encodeURIComponent(
          clientRunId
        )}&select=id`
      )
      const existing = rows[0]
      if (existing) {
        return { kind: 'existing', runId: String(existing.id) }
      }
    }
    throw err
  }
...
interface ViaLangGraphArgs {
  ...
  /** Pre-generated run id; used when a `clientRunId` row was reserved upfront. */
  runId: string
  /** Timestamp generated by the caller and used for the reserved row. */
  startedAt: IsoTimestamp
  startedAtMs: number
}

async function executeViaLangGraph(args: ViaLangGraphArgs): Promise<ExecuteCopilotRunResult> {
  const { copilotId, versionId, projectId, model, userInput, maxSteps, userLabel, clientRunId, runId, startedAt, startedAtMs } = args
  ...
  if (clientRunId !== null) {
    // The row was reserved with status='running' before execution. Patch it
    // with the final outcome rather than inserting a second row.
    await pgrest('PATCH', `agent_runs?id=eq.${runId}`, {
...
export async function executeCopilotRun(
  ...
  // Generate run identity and input metadata once, shared by both runtime paths.
  const startedAtMs = Date.now()
  const startedAt: IsoTimestamp = new Date(startedAtMs).toISOString()
  const inputSummary = summarize(userInput)
  const runId = randomUUID()

  // When the caller supplied an idempotency key, reserve the agent_runs row
  // BEFORE any model work. If the unique index (copilot_id, client_run_id)
  // already holds a row, we return a typed error without executing.
  if (clientRunId !== null) {
    ...
    if (reservation.kind === 'existing') {
      throw new DuplicateClientRunIdError(clientRunId, reservation.runId)
    }
  }
