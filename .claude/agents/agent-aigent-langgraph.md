...
agrégations → fiche backend. Front : il n'y en a pas (`AGENTS.md` § Frontend).

## Repo & validation

- Dev : `npm run dev` (`scripts/dev-stack.mjs` — Next **3987** + LangGraph **2024**) ;
  `npm run dev:next` / `npm run langgraph` pour une moitié. Jamais 3000 / 3001 / 3210.
...
- Next.js : lis `node_modules/next/dist/docs/` avant de coder du framework ; la garde d'identité
  est `src/proxy.ts` (matcher `/api/agent-ops/:path*`), pas un `middleware.ts`.
...
`confirmedToolNames`. **Pas ouvert aux runs produit** (la route de run le refuse) : il sert les
runners test/benchmark et `evidence/live-adapter.ts`.

**Plafond de coût réel des deux côtés** : `runner.ts` compare le coût cumulé avant chaque appel
modèle, et le graphe a le jumeau `COST_BUDGET_EXHAUSTED` à côté de `STEP_BUDGET_EXHAUSTED`.
Dans le graphe, `spentUsd` rend `null` sans taux transporté ou sans `usage_metadata`, et **rien
n'est alors imposé** — un plafond contre un tarif inventé serait pire que pas de plafond.
...
`langgraph.json` → `./src/langgraph/agent-builder-graph.mjs:graph`, state
`MessagesAnnotation`, boucle **agent → approval → tools → agent** (`routeAgent` /
`routeApproval`, `END` sans tool call).

- `agentNode` — n'instancie **plus** `ChatOpenAI` en direct : il passe par `createChatModel` /
  `bindChatModelTools` de `./model-provider.mjs`. Budget compté par `countAgentTurns` **depuis
  le state** (jamais un compteur module-level : le graphe compilé est un singleton partagé).
...
`routeCompletion` dispatche via `callProvider` ; `ModelProvider` vaut `'openai' | 'google' | 'local'`.
...
`providerAvailable` est la source unique du « ce provider est-il configuré ? » et le primaire
n'est tenté que s'il l'est. Fallbacks (`model-fallbacks.ts`) : cible OpenAI, `judge` toujours,
`run`/`benchmark`/`architect` seulement avec `AMC_ALLOW_MODEL_FALLBACKS=1` ou un opt-in de
requête, sinon fail-closed ; un fallback est toujours marqué. `model-pricing.ts` déclare des
**estimations**, pas une vérité de facturation ; `computeCostUsd` n'est jamais NaN.

## Streaming — précision, pas de généralisation

- Le **streaming de tokens** vit dans `project-builder-conversation.ts` (domaine lifecycle),
  qui appelle `getOpenAIClient()` + `ARCHITECT_MODEL` directement, **pas** via
  `routeCompletion`. Seul le tour de prose finale streame ; le scouting d'outils, non.
- `routeCompletion` **ne streame pas** et `/api/agent-ops/architect` est **stateless et
  non-streamée** (`{reply, manifest}`).
- `streamOnAgentServer` streame des **événements de nœuds** (`updates`) ; le mode `messages`
  est ignoré → pas de deltas de tokens, et le résultat final est relu du state terminal,
  identique au chemin bloquant.
- Formule à ne jamais écrire : « stream:true sur le dernier appel LLM » comme fait général.

## Env vars (noms réels, tous dans `.env.example`)

`LANGGRAPH_API_URL` · `AIGENT_LANGGRAPH_PORT` · `LANGGRAPH_SERVER_SECRET` ·
`AGENT_BUILDER_MODEL` · `OPENAI_API_KEY` · `GEMINI_API_KEY`/`GOOGLE_API_KEY` ·
`VLLM_LOCAL_API_KEY` + `VLLM_GPU*_URL`/`VLLM_GPU*_MODEL` · `AMC_ALLOW_MODEL_FALLBACKS` ·
`AMC_DATA_SOURCE=gpu1` · `GITHUB_TOKEN` (+ `GITHUB_PUSH_ENABLED`) ·
`AIGENT_RUNTIME_API_TOKEN` (`/api/runtime/v1/**`) · `AGENT_ENDPOINT`/`AGENT_API_KEY`
(runtime custom de `github.ts`).
...
- **Poser une question est permis** quand l'ambiguïté coûte cher : suppression importante,
  ...
  benchmark, matérialisation facturée). Une ligne, ton option recommandée, et tu avances sur ce
  qui n'en dépend pas.
- **Un typecheck vert ne prouve pas un run** : déclenche le chemin et observe l'état persisté,
  sinon écris « codé, non vérifié ». Tu rapportes fichiers édités, ce que tu as validé, ce qui
  reste ouvert.
