...
describe('legacy front — demolished admin routes stay demolished', () => {
  it.each(['agents', 'projects', 'factory', 'performance', 'settings', 'telemetry'])(
  ...
  it('only the two live placeholders and their layout remain directly under src/app/admin', () => {
    const entries = readdirSync(join(ROOT, 'src/app/admin'), { withFileTypes: true })
    const names = entries.map((e) => e.name).sort()
    expect(names).toEqual(['layout.tsx', 'page.tsx', 'runs'])
  })
})
